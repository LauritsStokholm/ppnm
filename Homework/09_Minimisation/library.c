...
/* Sum */
// Calculate a+b and stores result in a
void
...
/* Products */
// Calculate a*b and stores result in b;
void vector_scale (double a, vector* b)
{
  for (int k=0; k<b->size; k++)
  {
    vector_set (b, k, vector_get(b, k) * a);
  }
}

// Calculate v*w = v_transposed * w = sum_i (vi * wi)
void vector_dot_product (vector* v, vector* w, double* result)
{
...
void
vector_outerproduct (vector* v, vector* w, matrix* vw)
{
  assert (vw->size1 == v->size && vw->size2 == w->size);
  double vi, wj;
  for (int i=0; i<vw->size1; i++)
  {
    for (int j=0; j<vw->size2; j++)
    {
      vi = vector_get (v, i);
      wj = vector_get (w, j);
      matrix_set (vw, i, j, vi*wj);
    }
  }
}


// Calculates a*A and stores it in A
...
// Calculates A+B and stores it in A
...
{
  assert (src->size1 == dest->size2 && src->size2 == dest->size1);

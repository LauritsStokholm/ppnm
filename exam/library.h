...
/* ............................................................
 * DESCRIPTION: Within this libraryfile, essential constructable 
 * functions as well as simple algebraic functions for the
 * customised structures (vector and matrix) are defined.
 *
 * For glossary see main.h
 * ............................................................ */
...
void matrix_scale (matrix* A, double s)
{
  for (int i=0; i<A->size1; i++)
  {
    for (int j=0; j<A->size2; j++)
    {
      matrix_set (A, i, j, matrix_get(A, i, j)*s);
    }
  }
}

// Print structure to file pointer
...
// Copy given structure (original) to other destination (copy)
...
// View substructure of a structure
...
double vector_innerproduct (vector* v, vector* w)
{
  assert (v->size == w->size);
  double sum = 0;
  for (int i=0; i<v->size; i++)
  {
    sum += vector_get(v, i) * vector_get(w, i);
  }
  return sum;
}
...
void fprintf_result_matrix_matrix_test_equal (FILE* fp, matrix* A, matrix* B, double epsabs, double epsrel)
{
    ...
    {
      fprintf(fp, "Result: Yes, they are equal; using absolute = %lg \t relative = %lg precision\n", epsabs, epsrel);
    }
    else {fprintf(fp, "Result, No, they are not equal; using absolute = %lg \t relative = %lg precision.\n", epsabs, epsrel);}
}
